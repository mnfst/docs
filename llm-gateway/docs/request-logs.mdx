---
title: "Request logs"
description: "Inspect the request received from your client and the body and response for each provider attempt."
icon: "logs"
keywords:
...
The [Requests page](/llm-gateway/docs/observability) tells you what a request cost and which model served it. Request logs tell you what was actually said: the body the gateway sent to the provider, and the body that came back. The dashboard calls this feature **Logs**.
...
A request blocked by the gateway, on a [hard limit](/llm-gateway/docs/llm-gateway#hard-limits) or a malformed body, still lists one attempt, labelled with the model you asked for or **No provider**. It carries the failure status and no **Messages**, **Tools**, or **Raw** tab: the gateway sent nothing to a provider, so there is nothing to read back.
...
<Note>
  ...
  [upgrade the gateway](/llm-gateway/docs/self-hosted#upgrading) first.
</Note>
...
- [Data and telemetry](/llm-gateway/docs/reference/telemetry) — what the gateway stores by default
