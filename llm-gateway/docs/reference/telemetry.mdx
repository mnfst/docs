...
canonical: "https://manifest.build/llm-gateway/docs/reference/telemetry"
---
...
<Note>
  If a harness handles data you'd rather Manifest never hold onto, switch
...
| `messages_total` | `1284` | Daily activity per install. One count per provider attempt, so a request that fell back twice counts three. Requests Manifest blocked count too |
...
- [Observability](/llm-gateway/docs/observability) — what the recorded metadata gets you
- [Environment variables](/llm-gateway/docs/reference/environment-variables) — `MANIFEST_TELEMETRY_DISABLED` and `TELEMETRY_ENDPOINT` in context
- [Self-hosting with Docker](/llm-gateway/docs/self-hosted) — where to put these variables
