...
canonical: "https://manifest.build/llm-gateway/docs/reference/headers"
---
...
| Your custom tier header | your value           | Routes the request to a [custom tier](/llm-gateway/docs/llm-gateway#custom-tiers). You set the key and value when you create the tier, so the exact header name is up to you. |
...
The custom tier header is the only one that changes routing: send the header you configured on a tier and the request goes to that tier's model. The header wins even when the body names an explicit model ID. See [Routing → Custom](/llm-gateway/docs/llm-gateway#custom-tiers).
