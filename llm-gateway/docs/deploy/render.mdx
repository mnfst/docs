...
canonical: "https://manifest.build/llm-gateway/docs/deploy/render"
---
...
- The Blueprint mounts a 1 GB persistent disk `manifest-recordings` at `/data/request-recordings` and sets `REQUEST_RECORDING_STORAGE=filesystem`, so [request logs](/llm-gateway/docs/request-logs) survive redeploys. This disk is single-instance. Point recording at S3-compatible storage before you scale to more than one instance.
