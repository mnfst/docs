...
<Note>
  The built-in Ollama, LM Studio, and llama.cpp connections are self-hosted only. To reach a local model from [Manifest Cloud](https://app.manifest.build), expose the runtime on a public URL or tunnel and add it as a [custom provider](/llm-gateway/docs/providers/custom-providers).
</Note>
