...
User-supplied URLs are revalidated on every request to defend against SSRF. On Manifest Cloud, a custom provider must use a public `https://` URL. Self-hosted deployments also accept private addresses and plain `http://`, so they can reach a server inside your own network.
