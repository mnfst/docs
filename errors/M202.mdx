...
sidebarTitle: "M202"
description: "Manifest error M202 fires when one IP sends more than 500 requests per minute across all harnesses. Returns HTTP 429. Fix: stagger workers or raise the IP cap."
icon: "timer"
keywords:
...
There's a separate per-IP cap (500 requests per minute by default) sitting on top of the per-user limit. It catches abuse, plus dev setups where lots of agents share one IP. You hit it because that IP fired too many requests across all harnesses in a 60-second window.
