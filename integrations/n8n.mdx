...
- **n8n on another machine.** The bundled compose file binds the gateway to `127.0.0.1` only. Set `HOST_BIND_ADDRESS=0.0.0.0` in the gateway's `.env`, as described in [Exposing on the LAN](/self-hosted#exposing-on-the-lan), so the port answers on the network.
...
<Steps>
  ...
  <Step title="Pick the credential and model">
    Select the **Manifest API** credential. The model list is loaded from your instance. Leave it on `auto` to let your [routing rules](/llm-gateway#routing) pick the model, or choose a model ID to pin one.
  </Step>
