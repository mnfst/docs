...
The toggle lives on each harness's **Settings** page, under **Message recording**.

New harnesses have it on. Older harnesses keep whatever they were set to, because only the default changed, so a harness from before the feature shipped probably needs the toggle flipped once.
