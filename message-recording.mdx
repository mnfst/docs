...
description: "Store the full request and response body of every provider attempt, and read them back as a conversation in the request drawer. One toggle per harness."
...
The [request log](/observability) tells you what a request cost and which model served it. Message recording tells you what was actually said: the body Manifest sent to the provider, and the body that came back. The dashboard calls this feature **Logs**.
...
{/* TODO(Seb): screenshot — a request opened, Attempts list visible, Messages tab selected. Suggested path: /images/message-recording-drawer.png */}
...
{/* TODO(Seb): screenshot — the harness Settings page, Logs section with the Enable logs switch on. Suggested path: /images/message-recording-settings.png */}
