...
<Note>
  **Upgrading from before v6.18.0 and using [logs](/message-recording)?** Run the installer once with the upgrade flag, or log bodies are written inside the container and lost when it is recreated:
...
Manifest always keeps metadata about each request — model, provider, tier, token counts, cost, latency. Message bodies are separate and optional: with [message recording](/message-recording) on for a harness, prompts and completions are stored too, in the recordings volume rather than the database. New harnesses have it enabled. See [Data and telemetry](/reference/telemetry).
