...
To run the gateway somewhere other than your own machine, see the [other self-hosting paths](/deploy).
...
<Steps>
  ...
  <Step title="Send a request">
    ...
    `"model": "auto"` asks the gateway to route the request. Any other name is treated as an explicit choice: if the model isn't available to your harness, the request fails with [M302](/errors/M302). Send `auto` to use routing.
  </Step>
</Steps>

The request shows up in the dashboard straight away, with the model that served it and what it cost.

You can also do all of this from the terminal. See the [CLI](/cli), and on a self-hosted instance sign in with `mnfst login --url http://localhost:2099`.

<Note>
  Errors raised by the gateway itself carry an `M###` code, a plain-English cause, and a link to the matching page under [manifest.build/llm-gateway/docs/errors](/errors). The three you are most likely to see on a fresh install:
...
<Note>
  **Upgrading from before v6.18.0 and using [logs](/request-logs)?** Run the installer once with the upgrade flag, or log bodies are written inside the container and lost when it is recreated:
...
| `manifest_request_recordings` | `/data/request-recordings` in `manifest` | Stored [request logs](/request-logs) bodies |
...
Everything else is optional: provider timeouts, email delivery for alerts and password resets, OAuth logins, connection-pool sizing, recording storage and retention, Autofix, and Sentry. See [Environment variables](/reference/environment-variables) for the full list with defaults.
