...
<Tabs>
  ...
  <Tab title="Docker Run (BYO PostgreSQL)">
    ...
    The `127.0.0.1:` prefix keeps the dashboard on this machine. Drop it to reach the container from other machines on the network, and set `BETTER_AUTH_URL` to the address you type in the browser: the same rule as [Exposing on the LAN](#exposing-on-the-lan), which covers the compose install.
...
<Steps>
  ...
  <Step title="Send a request">
    ...
    `"model": "auto"` asks Manifest to route the request. Any other name is treated as an explicit choice: if the model isn't available to your harness, the request fails with [M302](/errors/M302). Send `auto` to use routing.
  </Step>
</Steps>
...
Everything else is optional: rate limits, provider timeouts, email delivery for alerts and password resets, OAuth logins, connection-pool sizing, recording storage and retention, Autofix, and Sentry. See [Environment variables](/reference/environment-variables) for the full list with defaults.
