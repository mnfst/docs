...
| `MANIFEST_MODE` | No | auto-detect | Set to `selfhosted` when Manifest doesn't recognise your install as self-hosted on its own. Docker, Podman, and Kubernetes are detected automatically; some platforms leave no trace, which is why the Render, Koyeb, Easypanel, and AWS guides set it |
