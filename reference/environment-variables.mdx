...
| `MANIFEST_MODE` | No | auto-detect | Forces the deployment mode: `selfhosted`, `local` (legacy alias), or `cloud`. Unset, Manifest treats a containerised runtime (Docker, Podman, Kubernetes) as self-hosted. Set `selfhosted` on platforms where the container leaves none of those traces — the deploy guides for Render, Koyeb, Easypanel, and AWS do |
