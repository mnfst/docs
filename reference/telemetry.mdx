...
Message bodies are separate, and optional. With [message recording](/message-recording) on for a harness, Manifest also stores the request and response bodies of each provider attempt so you can read them back in the dashboard. New harnesses have it enabled; harnesses created before the feature shipped keep whatever they were set to.
