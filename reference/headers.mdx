...
| `anthropic-version`     | `2023-06-01`         | Accepted and ignored. Anthropic SDKs send it on their own; Manifest sets the version it sends upstream itself.                                      |
...
The custom tier header is the only one that changes routing: send the header you configured on a tier and the request goes to that tier's model, even when the body names an explicit model ID — the header wins. See [Routing → Custom](/llm-gateway#custom-tiers).

## Response headers (routed requests)

| Header                       | Description                                                                      | Example         |
| ---------------------------- | -------------------------------------------------------------------------------- | --------------- |
| `X-Manifest-Tier`            | The route family that handled the request: `standard` for a custom-tier header match, `direct` for an explicit model ID, `simple` when Manifest answered without calling a provider | `direct`        |
...
| `X-Manifest-Reason`          | Why that route was picked: `header-match`, `direct`, and a set of routing reasons such as `default` — the list is not exhaustive | `header-match`  |
