...
When a fallback succeeds, the response carries `X-Manifest-Fallback-From` (the primary that failed) and `X-Manifest-Fallback-Index` (its position in the chain) on top of the standard routing headers. When the chain is exhausted, `X-Manifest-Fallback-Exhausted: true` is set with body code `fallback_exhausted`, and the response keeps the primary model's real error status (for example 500 or 529). Full table: [Headers reference](/reference/headers).
