...
<Steps>
  <Step title="Authentication">
    The `Authorization: Bearer mnfst_...` header resolves to one of your agents. A bad or missing key stops here with a [401](/errors).
  </Step>
  <Step title="Limits">
    If a hard limit for this agent is already over its threshold, the request is blocked with [M200](/errors/M200) before any provider is called. Nothing is spent.
  </Step>
  <Step title="Routing">
...
A limit rule can block requests as well as email you. When a rule's action includes blocking and the agent is over its threshold for the current period, the gateway rejects the request with [M200](/errors/M200) and HTTP `429`:
...
Rules are defined per agent, with a metric, a threshold, and a period. See [Spend alerts](/observability#spend-alerts) for how to create one and how the alerting half works.
