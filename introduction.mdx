...
On Cloud there is nothing to install or configure on a server. You sign up, connect your providers, and point your harness at the gateway URL. Routing, limits, [Autofix](/autofix), and alerts are settings in the dashboard. The [environment variables](/reference/environment-variables) throughout these docs apply only to a self-hosted gateway.
...
Install the gateway first. [Self-hosted](/self-hosted) covers Docker and the managed platforms. Your instance starts empty. [First request](/self-hosted#first-request) takes you from there to a routed request against `http://localhost:2099/v1`.
