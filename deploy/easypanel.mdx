...
Add an app volume named `recordings` mounted at `/data/request-recordings` so [request logs](/request-logs) survive restarts and redeploys. This volume is single-instance; move to S3-compatible storage before running more than one replica.
