...
- The deploy script creates a private Tigris bucket, stores its credentials as Fly secrets, and sets `REQUEST_RECORDING_STORAGE=s3` with the Tigris endpoint. [Request logs](/request-logs) persist across deploys and across scaled Machines. Nothing to attach manually.
