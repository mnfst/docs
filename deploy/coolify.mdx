...
- Persistent Docker volume for request logs, mounted at `/data/request-recordings`.
- Generated session, encryption, database password, and public URL values.

## Production notes

- Back up both Docker volumes through Coolify or your server provider: PostgreSQL data and the request logs volume at `/data/request-recordings`.
