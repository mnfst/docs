...
| **Provider attempt** | One call from Manifest to a provider | Every fallback step and every [Auto-fix](/autofix) retry adds another |
...
That's all metadata. To read the messages themselves, turn on [message recording](/message-recording) for the harness. The drawer picks up a **Messages** tab next to **Attempts**, with the body sent to the provider and the body that came back.
