...
A request that succeeds on the first try is one Request with one attempt. A request that fails twice before succeeding is still one Request, with three attempts. A request Manifest blocked itself, like a [hard limit](/llm-gateway#hard-limits) or a malformed body, is one Request with **zero** attempts, since no provider was ever called.
...
Pick a provider on the harness's **Limits** page: Resend, Mailgun, or SendGrid. Paste the API key and the notification address, then save. The saved provider appears as a card on the same page, where you can edit or remove it.

You can set the same thing instance-wide with the [`EMAIL_*` environment variables](/reference/environment-variables): `EMAIL_PROVIDER` (`resend`, `mailgun`, or `sendgrid`), `EMAIL_API_KEY`, `EMAIL_FROM`, and `EMAIL_DOMAIN` for Mailgun. A provider saved in the dashboard takes precedence over the environment variables.
